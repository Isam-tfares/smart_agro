...
const HumidityDetail = ({ navigation }) => {
    ...
    const fetchData = async (chosenDate) => {
        ...
        const url = filterType === 'day'
            ? `http://192.168.8.104:5000/data/day/${year}/${month}/${day}/Humidity`
            : `http://192.168.8.104:5000/data/month/${year}/${month}/Humidity`;
        // console.log(url)
        try {
            ...
            if (response.ok) {
                ...
                if (filterType === 'day') {
                    ...
                    setHumiditys(extractHumiditys(data));
                } else {
    ...
    const extractHumiditys = (data) => {
        if (data == null) { return []; }

        return data.map(entry => entry.Humidity);
    };
    const extractDaysAndTemp = (data) => {
        ...
        temp = extractHumiditys(data);
        function calculateAverageHumidity(day, Humiditys) {
            const tempsForDay = Humiditys.filter((_, index) => all_days[index] === day);
        ...
        const new_temp = uniqueDays.map(day => calculateAverageHumidity(day, temp));
        setDays(uniqueDays)
        setHumiditys(new_temp)
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <Text style={styles.title2}>Humidité</Text>
