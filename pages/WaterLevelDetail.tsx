...
const WaterLevelDetail = ({ navigation }) => {
    const [chosenDate, setChosenDate] = useState(new Date());
    const [showDatePicker, setShowDatePicker] = useState(false);
    const [filterType, setFilterType] = useState('day');
    const [hours, setHours] = useState([]);
    const [days, setDays] = useState([]);
    const [water_levels, setwater_levels] = useState([]);
    const [water_levelData, setwater_levelData] = useState([]);
    ...
    const fetchData = async (chosenDate) => {
        ...
        const url = filterType === 'day'
            ? `http://192.168.8.104:5000/data/day/${year}/${month}/${day}/water_level`
            : `http://192.168.8.104:5000/data/month/${year}/${month}/water_level`;
        // console.log(url)
        try {
            ...
            if (response.ok) {
                ...
                // console.log(data)
                setwater_levelData(data);
                if (filterType === 'day') {
                    setHours(extractHours(data));
                    setwater_levels(extractwater_levels(data));
                } else {
    ...
    const extractwater_levels = (data) => {
        if (data == null) { return []; }

        return data.map(entry => entry.water_level);
    };
    const extractDaysAndTemp = (data) => {
        ...
        temp = extractwater_levels(data);
        function calculateAveragewater_level(day, water_levels) {
            const tempsForDay = water_levels.filter((_, index) => all_days[index] === day);
        ...
        const new_temp = uniqueDays.map(day => calculateAveragewater_level(day, temp));
        setDays(uniqueDays)
        setwater_levels(new_temp)
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <Text style={styles.title2}>Niveau d'eau</Text>
